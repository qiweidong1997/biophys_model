...
vi= -65     # Inhibitory
vq= -65     # Readout
# ve_tmp= -65     # Excitatory
# vi_tmp= -65     # Inhibitory
# vq_tmp= -65     # Readout
...
for i in np.arange(numPools): ###and we should probably not use np.arange() but just normal [1:10,1:5] instead.
    ...
    See_row_start = int(Ne - np.dot(((numPools - 1) / numPools), Ne) + np.dot(Nre, (i - 1)))
    See_row_end   = int(Ne - np.dot(((numPools - 1) / numPools), Ne) + np.dot(Nre, i))
    See_col_start = int(np.dot(Nce, (i - 1)))
    See_col_end   = int(np.dot(Nce, i))
    See[See_row_start:See_row_end, See_col_start:See_col_end] = ee
    #print(See[See_row_start:See_row_end, See_col_start:See_col_end])

    #if i == 0:
    print(See_row_start, See_row_end, See_col_start, See_col_end)

    # Synapses _ excitatory - inhibitory
    Sei_row_start = int(np.dot(Nri,(i - 1)))
    Sei_row_end   = int(np.dot(Nri,i))
    Sei_col_start = int(np.dot(Nce,(i - 1)))
    Sei_col_end   = int(np.dot(Nce,i))
    Sei[Sei_row_start:Sei_row_end, Sei_col_start:Sei_col_end]= ei

    # Synapses _ inhibitory - excitatory
    Sie_row_start = int(Ne - np.dot(((numPools - 1) / numPools),Ne) + np.dot(Nre,(i - 1)))
    Sie_row_end   = int(Ne - np.dot(((numPools - 1) / numPools),Ne) + np.dot(Nre,i))
    Sie_col_start = int(np.dot(Nci,(i - 1)))
    Sie_col_end   = int(np.dot(Nci,i))
    Sie[Sie_row_start:Sie_row_end, Sie_col_start:Sie_col_end]=ie

See[0:int(Nre), int(np.dot((numPools - 1),Nce)):int(np.dot(numPools,Nce))]= ee
Sei[int(np.dot((numPools - 1),Nri)):int(np.dot(numPools,Nri)), int(np.dot((numPools - 1),Nce)):int(np.dot(numPools,Nce))]= ei
Sie[0:int(Nre), int(np.dot((numPools - 1),Nci)):int(np.dot(numPools,Nci))]= ie
...
for t in np.arange(0, (np.dot((1 / dt), simtime))): #1000 ms
    ...
    #firingsi=np.concatenate([[firingsi],[t + np.dot(0,firedi), firedi]])
    #firingsq=np.concatenate([[firingsq],[t + np.dot(0,firedq), firedq]])
    #Let all the fired neurons to go to rest
    ve_tmp[firede]=V_reset
    vi_tmp[firedi]=V_reset
    vq_tmp[firedq]=V_reset
    #start_time = time.time()
    if (firede.size != 0): #If firede is not empty #!!! ERROR: updates here might be wrong, causing long running time and wrong data

        print('t!=0')
        for i in np.arange(0,np.size(firede)):
            ...
            for j in np.arange(0, Ne):
                ...
                #firingse_update_mean = np.append(firingse_update_mean,
                #                                 np.mean(Ie[j, t + z: t + z + S_E.size] + np.dot(S_E, See[j, n])))
                #print(str(t) + ', ' + str(i) + ', ' + str(j));
                #if t == 60 and i == 0 and j == 0:
                #    print(Ie)
                #    sys.exit()
    ...
    if (firedi.size != 0):
        for i in np.arange(0,np.size(firedi)):
            n=firedi[i]
            for j in np.arange(0,Ne):
                z=ax_delayi[j,n]
